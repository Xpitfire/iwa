...
MOONS = ['0_src-1_tgt']
AMAZON_REVIEWS = ['books_src-dvd_tgt',
...
MINI_DOMAIN_NET = ['real_src-sketch_tgt',
...
METHOD_ADATIME = ['DDC', 'DANN', 'Deep_Coral', 'DSAN', 'MMDA', 'AdvSKM', 'HoMM', 'DIRT', 'CDAN', 'CoDATS', 'CMD']
...
def extract_results(dirs, da_method, dataset):
    ...
    iwv_preds = {}
    for d in dirs:
        if da_method in d[0] and 'seed' in d[0] and f'cls_pred_dataset_{dataset}.npz' in d[2]:
                try:
...
def run():
    ...
    if ds == 'HHAR_SA':
        ...
        ds_names = MOONS
        extractor = 'extractor/adatime_twinmoons_results_loader.py'
    elif ds == 'AMAZON_REVIEWS':
        ds_names = AMAZON_REVIEWS
        extractor = 'extractor/adatime_amazon_results_loader.py'
    elif ds == 'MINI_DOMAIN_NET':
        ds_names = MINI_DOMAIN_NET
        extractor = 'extractor/adatime_minidomainnet_results_loader.py'
    else:
