...
class VAT(nn.Module):
    def __init__(self, model, device, xi=1e-6, eps=3.5, n_power=1):
...
class LMMD_loss(nn.Module):
    ...
    def get_loss(self, source, target, s_label, t_label):
        batch_size = batch_size = min(source.shape[0], target.shape[0])
    ...
    def cal_weight(self, s_label, t_label, batch_size, class_num=4):        
