...
from .aux_da_losses import F_mmd_loss, F_cmd_loss
...
def domain_adversarial_no_class(config, output, s_class_target, t_class_target):
    """Computes cross entropy and adversarial loss based on the source and target data (without source and target labels).
    Based on the domain Domain Adversarial Neural Network paper https://arxiv.org/abs/1505.07818 
    """
    class_logits, domain_logits = output
    batchsize = class_logits.shape[0] // 2
...
def iwv(config, output, s_class_target, t_class_target):
    """Loss for domain classifier. Creates source-target labels on the fly. 
    Source has label 0, Target has label 1.
    """
    domain_logits, _ = output
    batchsize = domain_logits.shape[0] // 2
...
def iwv_incl_source(config, output, s_class_target, t_class_target):
    """Loss for domain classifier with auxiliary sources. Creates source-target labels on the fly. 
    Source has label 0, Target has label 1.
    """
    domain_logits, class_logits = output
    batchsize = domain_logits.shape[0] // 2
    ...
    # collect the loss that will be used for optimization
    loss = s_domain_loss + t_domain_loss + 0.1*s_class_loss
    return loss, {'loss': loss.item(), 's_domain_loss': s_domain_loss.item(), 't_domain_loss': t_domain_loss.item(), 's_class_loss': s_class_loss.item()}
