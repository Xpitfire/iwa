...
class CNN2D(nn.Module):
    def __init__(self, configs):
        ...
        self.conv_block1 = nn.Sequential(
            nn.Conv2d(configs.input_channels, configs.mid_channels, kernel_size=configs.kernel_size,
                      stride=configs.stride, bias=False, padding=(configs.kernel_size // 2)),
            nn.BatchNorm2d(configs.mid_channels),
            nn.ReLU(),
        ...
        self.conv_block2 = nn.Sequential(
            nn.Conv2d(configs.mid_channels, configs.mid_channels * 2, kernel_size=8, stride=1, bias=False, padding=4),
            nn.BatchNorm2d(configs.mid_channels * 2),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2, padding=1)
        )

        self.conv_block3 = nn.Sequential(
            nn.Conv2d(configs.mid_channels * 2, configs.mid_channels * 2, kernel_size=8, stride=1, bias=False,
                      padding=4),
            nn.BatchNorm2d(configs.mid_channels * 2),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2, padding=1),
        ...
        self.conv_block4 = nn.Sequential(
            nn.Conv2d(configs.mid_channels * 2, configs.final_out_channels, kernel_size=8, stride=1, bias=False,
                      padding=4),
        ...
        self.adaptive_pool = nn.AdaptiveAvgPool2d(configs.features_len)
...
class MLP(nn.Module):
    def __init__(self, configs):
        ...
        self.mlp_block1 = nn.Sequential(
            nn.Linear(configs.input_channels, configs.model.n_hidden),
            nn.BatchNorm1d(configs.model.n_hidden),
            nn.ReLU(),
            nn.Dropout(configs.dropout)
        )
        self.mlp_block2 = nn.Sequential(
            nn.Linear(configs.model.n_hidden, configs.model.n_hidden),
            nn.BatchNorm1d(configs.model.n_hidden),
            nn.ReLU(),
            nn.Dropout(configs.dropout)
        )
        self.out_layer = nn.Linear(configs.model.n_hidden, configs.final_out_channels)

    def forward(self, x_in):
        x = self.mlp_block1(x_in)
        x = self.mlp_block2(x)
        x = self.out_layer(x)
        return x


class Pretrained2D(nn.Module):
    def __init__(self, configs):
        super(Pretrained2D, self).__init__()
        self.configs = configs
        self.backbone, self.backbone_model, params, in_features = get_backbone(configs)
        self.adaptive_pool = nn.AdaptiveAvgPool2d(output_size=configs.backbone.avgpool)
        self.flatten = nn.Flatten()
        self.out_layer = nn.Linear(in_features, configs.final_out_channels)

    def forward(self, x_in):
        if self.configs.used_da_method == 'DIRT':
